...
import { translatePromptAToFullAnalysis } from '@/lib/llm/translatePromptAToFullAnalysis';
import { validateAndSanitizeAnalysis } from '@/lib/validation';

describe('translatePromptAToFullAnalysis', () => {
  it('maps LLM output into the validated analysis shape', () => {
    const raw = {
    ...
    expect(validated.timeline.segments.length).toBeGreaterThan(0);
